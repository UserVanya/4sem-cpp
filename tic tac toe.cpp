...
void makeAMove(int currDepth, map<int, int> currCollocation, int lastStep, set<map<int, int>>& comb, int& amount) {
	if (thisCollocIsWinning(currCollocation)) {
		comb.insert(currCollocation);
		amount++;
		return;
	}
	if (currDepth == lastStep) {
		for (int i = 0; i < 9; i++) {
	...
	if (currDepth % 2 == 0) {
		for (int i = 0; i < 9; i++) {
			if (currCollocation[i + 1] == 0) {
				currCollocation[i + 1] = X;
				makeAMove(currDepth + 1, currCollocation, lastStep, comb, amount);
				currCollocation[i + 1] = 0;
			}
		}
	}
...
int main() {
	...
	int amount = 0;
	makeAMove(0, currCollocation, 8, comb, amount);
